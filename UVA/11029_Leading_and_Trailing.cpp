...
void solve(){
    ...
    ld power = (ld)k * log10(n) ;
    ld firstDigit = pow(10, power - floor(power)) * 100 ;
    string lastDigits = to_string(binpowMod(n , k , 1000ll)) ;
    while(sz(lastDigits) < 3) lastDigits = "0" + lastDigits ;
    cout << (int)firstDigit << "..." << lastDigits << '\n' ;
}
int32_t main(){   
